...
import { RegisterPayload, LoginPayload, StoredUser, UserProfile, DbStatusResponse } from './src/types.ts';
import pg from 'pg';
...
export function createExpressApp() {
  ...
  app.get('/api/db/status', async (req: Request, res: Response) => {
    ...
    const directPg = getDirectPgConnectionString();
    ...
    const responseData: DbStatusResponse = {
      ...
      sqlEditorUrl,
      directPgConfigured: Boolean(directPg),
    };

    res.json(responseData);
  });

  // Trigger automated SQL initialization if direct Postgres connection is available
  app.post('/api/db/init-table', async (req: Request, res: Response) => {
    const directPg = getDirectPgConnectionString();

    if (directPg) {
      const pgClient = new pg.Client({
        connectionString: directPg,
        ssl: { rejectUnauthorized: false },
      });

      try {
        await pgClient.connect();
        await pgClient.query(SUPABASE_SCHEMA_SQL);
        await pgClient.end();
        ...
        res.json({
          success: true,
          tableExists: testResult.tableExists,
          message: 'Successfully executed SQL schema via PostgreSQL connection! Table "users" is now active.',
        });
        return;
      } catch (pgErr: any) {
        console.error('Direct PG execution error:', pgErr);
        res.status(500).json({
          success: false,
          message: `Direct PostgreSQL execution failed: ${pgErr.message}. Please copy the SQL into Supabase SQL Editor.`,
        });
        return;
      }
    }

    // If no direct PG connection string, recheck Supabase client
    const testResult = await testSupabaseConnection();
